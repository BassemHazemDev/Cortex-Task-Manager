...
const QuickTodosCard = ({ onOpenTodoForm, onEditTodo }) => {
  ...
  return (
    ...
    >
      ...
      <CardContent>
        {/* Show "All caught up!" when no incomplete todos exist */}
        {todos.filter(t => !t.isCompleted).length === 0 ? (
          <>
            <EmptyTodoList onAddTodo={() => onOpenTodoForm()} />
            {/* Still show completed todos section if there are any */}
            {todos.length > 0 && (
              <div className="mt-4">
                <button
                ...
                {showCompletedTodos && (
                  <div className="mt-3 space-y-2">
                    ...
                    {Math.ceil(todos.length / completedTodosPerPage) > 1 && (
                      <div className="flex justify-center items-center gap-3 mt-4 pt-3">
                        ...
                        <span className="text-xs font-medium px-3 py-1 rounded bg-muted text-muted-foreground">
                          {completedTodosPage} / {Math.ceil(todos.length / completedTodosPerPage)}
                        </span>
                        <Button
                          variant="outline"
                          size="sm"
                          ...
                          onClick={() =>
                            setCompletedTodosPage((p) =>
                              Math.min(Math.ceil(todos.length / completedTodosPerPage), p + 1)
                            )
                          }
                          disabled={
                            completedTodosPage === Math.ceil(todos.length / completedTodosPerPage)
