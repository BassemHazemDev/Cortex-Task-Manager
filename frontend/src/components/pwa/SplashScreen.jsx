...
const SplashScreen = ({ isReady, onComplete }) => {
  ...
  useEffect(() => {
    if (isReady && show) {
      // Trigger exit animation when auth is ready
      setShow(false);
  ...
  // Show loading animation while waiting for auth
  const shouldShow = !isReady || show;
      ...
      {shouldShow && (
        ...
        >
          ...
          <div className="relative z-10 flex flex-col items-center">
            ...
            {/* Loading Bar - only show when waiting for auth */}
            {!isReady && (
              <motion.div
