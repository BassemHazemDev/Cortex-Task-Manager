...
const TaskForm = ({ task, initialDate, onSave, onCancel }) => {
  ...
  // State for duration unit (minutes or hours) - initialized before dirty checking
  const [durationUnit, setDurationUnit] = useState(() => {
    const duration = task?.estimatedDuration || 60;
    // Default to hours if duration is >= 60 and evenly divisible by 60
    return duration >= 60 && duration % 60 === 0 ? "hours" : "minutes";
  });

  // Track initial form state for dirty checking (includes durationUnit)
  const initialFormDataRef = useRef(null);
  const initialDurationUnitRef = useRef(null);
  useEffect(() => {
    // Capture initial state on mount
    if (initialFormDataRef.current === null) {
      initialFormDataRef.current = JSON.stringify(formData);
      initialDurationUnitRef.current = durationUnit;
    }
  ...
  // Compute if form has unsaved changes (includes durationUnit change)
  const isDirty = useCallback(() => {
    if (!initialFormDataRef.current) return false;
    const formChanged = JSON.stringify(formData) !== initialFormDataRef.current;
    const unitChanged = durationUnit !== initialDurationUnitRef.current;
    return formChanged || unitChanged;
  }, [formData, durationUnit]);

  // Handle attempt to close the form
  ...
  const handleSubmit = (e) => {
    ...
    // Sanitize duration - formData already stores the value in minutes
    // (conversion happens in onChange when unit is hours)
    ...
    if (dueDate) {
      ...
      dueDate = `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(
        d.getDate()
      )}`;
      if (dueTime) {
  ...
  return (
    <div className="fixed inset-0 bg-accent/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <Card
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  placeholder={
                    formData.descriptionType === "list"
                      ? "- Item 1\n- Item 2"
                      : formData.descriptionType === "chunks"
                      ? "[Section]\nContent..."
                    ...
                    value={
                      durationUnit === "hours"
                        ? formData.estimatedDuration >= 60
                          ? Math.round(formData.estimatedDuration / 60)
                          : formData.estimatedDuration
                        : formData.estimatedDuration
                    }
                    onChange={(e) => {
                      const val = parseInt(e.target.value) || 0;
                      // Store the raw value; conversion happens at submit
                      if (durationUnit === "hours") {
                        handleChange("estimatedDuration", val * 60);
                      } else {
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  {formData.tags.map((tag) => (
                    <span
                      key={tag}
                      className={`px-2 py-1 rounded flex items-center gap-1 text-xs border ${getTagColorClass(
                        tag
                      )}`}
                    >
                      {tag}
                      <button
                        ...
                        className="ml-1 text-inherit opacity-70 hover:opacity-100 font-bold"
                      >
                        ×
                      </button>
                    </span>
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    handleChange("repeatFrequency", value)
                  }
                >
                  <SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                />
                <p className="text-sm text-gray-500">
                  If set, this task will repeat according to the selected
                  frequency until the chosen date.
                </p>
              </div>
