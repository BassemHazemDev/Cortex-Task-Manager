  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="w-full max-w-[calc(100%-2rem)] sm:max-w-md rounded-xl max-h-[90vh] overflow-y-auto gap-0 p-0 overflow-hidden bg-card mx-auto">
                    ...
                    <div className="flex items-center gap-2">
                        <span className={cn(
                            "inline-flex items-center px-2 py-0.5 rounded text-xs font-medium border",
                             item.priority === 'high' ? "bg-red-50 text-red-700 border-red-200 dark:bg-red-950/50 dark:text-red-400 dark:border-red-900" :
                             item.priority === 'medium' ? "bg-yellow-50 text-yellow-700 border-yellow-200 dark:bg-yellow-950/50 dark:text-yellow-400 dark:border-yellow-900" :
        ...
        <div className="p-6">
            {isTask ? (
                <TaskDetailView task={item} onToggleSubtask={onToggleSubtask} />
            ) : (
        ...
        <div className="px-6 py-4 bg-muted/20 border-t flex justify-between items-center gap-4">
            {showDeleteConfirm ? (
                <div className="flex items-center gap-2 w-full justify-end animate-in fade-in slide-in-from-right-4 duration-200">
                    <span className="text-sm font-medium text-destructive mr-2">Are you sure?</span>
                    <Button 
                        variant="ghost" 
                        size="sm" 
